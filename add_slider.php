<?php 
 ...
 // Get add event slider & external
  if(isset($_POST['submit']) and isset($_GET['add']))
  {
      ...
      if($slider_type=='external'){
          ...
          switch ($slider_type) {
            case 'Event':
                ...
                $edit_status=Update('tbl_events', $data_status, "WHERE id = ".$event_id);
              break;
      ...
      if($event_id!=0){
        $sql="SELECT * FROM tbl_slider WHERE `event_id`='$event_id' AND `slider_type`='$slider_type' AND `status`='1'";
              ...
              <div class="section">
                <div class="section-body">
                  ...
                  <div class="form-group video_status">
                    <label class="col-md-3 control-label" for="id">Event :-</label>
                    <div class="col-md-6">
                      <select name="event_id" id="event_id" class="select2" required="">
                        <option value="">--Select Event--</option>
                         <?php
                          $sql="SELECT * FROM tbl_events WHERE tbl_events.`status`='1' ORDER BY `id` DESC";
                          $res=mysqli_query($mysqli, $sql);
                          while($row=mysqli_fetch_array($res))
                          {
                        ?>                       
                          <option data-url="images/<?php echo $row['event_banner'];?>" value="<?php echo $row['id'];?>"><?php echo $row['event_title'];?></option>                           
                        <?php
                          }
