<?php 
    ...
    $sql="SELECT * FROM tbl_events WHERE id='$event_id' ORDER BY tbl_events.`id` DESC";
    ...
    $url2='event://event_app/view_event.php?event_id='.$event_id;

    $download_url='https://play.google.com/store/apps/details?id=com.einvie.app';
...
<div class="container">
    ...
    <h3>Event Name: <span style="font-weight:500;color:#424242;font-size:16px"><?=$row['event_title']?></span></h3>
    <h3>Start Date & Time: <span style="font-weight:500;color:#424242;font-size:16px"><?=date('m/d/Y', $row['event_start_date'])?></span></h3>
    <h3>End Date & Time: <span style="font-weight:500;color:#424242;font-size:16px"><?=date('m/d/Y', $row['event_end_date'])?></span></h3>
    <h3>Website: <span style="font-weight:500;color:#424242;font-size:16px"><?=$row['event_website']?></span></h3>
    <!--<img class="add_images" src="banner.jpg" alt="banner" />-->
    <p style="font-weight:500">If you already have the install app then View event details</P>
    <button class="joinBtn" onclick="window.open('<?=$url2?>','_blank')">View event details!</button>
